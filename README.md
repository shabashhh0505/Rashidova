...
1. Перепроверить местонахождение файла prometheus.yaml через терминал.
2. Перепроверить правильно ли написан код графаны (1860) и код прометеуса (http://prometheus:9090)
3. После выполнения вышеперечисленных пунктов, перезапускаем врм.
